...
class PMFLayerTestCase(unittest.TestCase):
    ...
    def test_pmf_logit_to_probs_zeros(self):
        ...
        expected_s = torch.tensor([[1 - np.exp(0)/(np.exp(0)*5+1) * (i+1) for i in range(5)]], dtype=torch.float32)
    ...
    def test_pmf_logit_to_probs_ones(self):
        ...
        expected_s = torch.tensor([[1 - np.exp(1)/(np.exp(1)*5+1) * (i+1) for i in range(5)]], dtype=torch.float32)
