...
def wrapped_c_index(data, censoring_dist, max_followup=5):
    probs = data[[f"year_{i + 1}" for i in range(max_followup)]].to_numpy()
    time_to_event = data["time_to_event"].clip(upper=max_followup - 1)
    cancer = data["cancer"]
    return concordance_index(time_to_event, probs, cancer, censoring_dist)
...
def _concordance_summary_statistics(
    ...
    while True:
        ...
        if has_more_censored and (not has_more_died or died_truth[died_ix] > censored_truth[censored_ix]):
            pairs, correct, tied, next_ix, weight = _handle_pairs(censored_truth, censored_pred, censored_ix,
                                                                  times_to_compare, censoring_dist)
            censored_ix = next_ix
        elif has_more_died and (not has_more_censored or died_truth[died_ix] <= censored_truth[censored_ix]):
            pairs, correct, tied, next_ix, weight = _handle_pairs(died_truth, died_pred, died_ix, times_to_compare,
                                                                  censoring_dist)
...
class CIndex(AbstractFunction):
    def __init__(self, function=wrapped_c_index):
        super().__init__()
        self.name = "C-Index"
