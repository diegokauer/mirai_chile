...
def get_annotation_mask(x, batch, volatile, args):
    ...
    _left_x = torch.min(batch['region_bottom_left_x'], batch['region_top_left_x']).unsqueeze(1).unsqueeze(-1).unsqueeze(
        -1).float()
    _right_x = torch.max(batch['region_bottom_right_x'], batch['region_top_right_x']).unsqueeze(1).unsqueeze(
        -1).unsqueeze(-1).float()
    left_x = torch.min(_left_x, _right_x)
    right_x = torch.max(_left_x, _right_x)
    _top_y = torch.min(batch['region_top_left_y'], batch['region_top_right_y']).unsqueeze(1).unsqueeze(-1).unsqueeze(
        -1).float()
    _bottom_y = torch.max(batch['region_bottom_left_y'], batch['region_bottom_right_y']).unsqueeze(1).unsqueeze(
        -1).unsqueeze(-1).float()
...
def get_region_annotation_for_sample(sample, region_annotations, args):
    if args.multi_image:
        return [get_region_annotation_for_path(path, region_annotations, args, indx) for indx, path in
                enumerate(sample['paths'])]
    else:
    ...
    if path in region_annotations:
        ...
        if len(annotations) > 0:
            ...
            if 'align_to_left' in args.image_transformers and IMAGE_RIGHT_ALIGNED[path]:
                ...
                region_annotation = flip_region_coords_left_right({'region_annotation': region_annotation})[
                    'region_annotation']
