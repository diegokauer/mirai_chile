...
from mirai_chile.models.utils.region_annotation import flip_region_coords_left_right, flip_region_coords_top_bottom, \
    rotate_region_coords_angle, make_region_annotation_blank
        ...
        if label == 0:
            ...
            if zoom_factor > 1:
                return self.random_cropper(
                    img.crop((x1 - patch_width // 2, y1 - patch_width // 2, x1 + patch_width // 2,
                              y1 + patch_height // 2)))
            else:
                return self.patch_scaler(img.crop((x1 - patch_width // 2, y1 - patch_width // 2, x1 + patch_width // 2,
                                                   y1 + patch_height // 2)))
